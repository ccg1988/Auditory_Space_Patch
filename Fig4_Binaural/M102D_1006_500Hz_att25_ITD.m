...
clear;clc;close all
load('M102D_1006_Tone_0.5kHz_Att25dB_F_mean.mat')
[X_pix, Y_pix, ~] = size(F_mean); 
Bin_type = 'ITD' ; %SpectralCues------ILD------ITD
attL = '25' ;          
ss = -300: 50: 300 ; % later on ITD stimuli             
...
F = reshape(F_mean, X_pix, Y_pix, trace_length, S1); % X--Y--mean trace--S
...
saveas( gcf, strcat('M102D_1006_500Hz_', Bin_type, '_',dF_type, '_dF',' base_type_', base_type,'_Att_', attL, ...
