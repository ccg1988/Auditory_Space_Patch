...
clear;clc;close all
load('M102D_0617_att40_ITD_4_Tone_R_mean.mat')
...
all_S= num2str(ss') ;
ITI = 1.4 ;               % Data acquisition time in "second"
fps = 10 ;
trace_length = ITI*fps;
pre_stim = 0.3 ;
base_range = 1 : pre_stim*fps ;  base_type = 'indi'; % 'indi' or 'toge'
dF_type = 'peak' ; % 'peak' or 'sum'
peak_range = (pre_stim+1/fps)*fps : (ITI-0.0)*fps ; 
sumF_range = (pre_stim+1/fps)*fps : (ITI-0.0)*fps ; %stop point could be determined by plotting raw F traces
F = reshape(R_mean, X_pix, Y_pix, trace_length, 4, S1); % X--Y--mean trace--S
%%
dF = F_to_dF (F(:, :, :, 1, :), base_range, base_type, dF_type, peak_range, sumF_range) ;
%%
...
saveas( gcf, strcat('M102D_0617_500Hz_', Bin_type, '_',dF_type, '_dF',' base_type_', base_type,'_Att_', attL, ...
